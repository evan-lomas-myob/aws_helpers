...
describe AwsHelpers::Actions::S3 do
  ...
  s3_bucket_name = 'myob-resources'
  s3_bucket_location = 'ap-southeast-2'
  s3_bucket_url = 'https://myob-resources.s3-ap-southeast-2.amazonaws.com'
  ...
  describe '#stack_exists?' do
    ...
    it 'should check the s3 bucket location' do
      expect(s3_location(config, s3_bucket_name)).to eq(s3_bucket_location)
    end

    it 'should return the s3 bucket url' do
      expect(s3_url(config, s3_bucket_name)).to eq(s3_bucket_url)
    end
