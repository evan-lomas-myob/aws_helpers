...
describe SnapshotsDelete do
  ...
  it '#snapshot_delete with db_instance supplied' do
    ...
    allow(SnapshotsDelete).to receive(:new).with(config, db_instance_id, default_days, default_months, default_years).and_return(snapshots_delete)
    expect(snapshots_delete).to receive(:execute)
    RDS.new(options).snapshots_delete(db_instance_id: db_instance_id)
  end

  it '#snapshot_delete with days, months and years supplied' do
    # need to add cases where days, months, years not supplied
    allow(Config).to receive(:new).with(options).and_return(config)
    allow(SnapshotsDelete).to receive(:new).with(config, db_instance_id, days, months, years).and_return(snapshots_delete)
    expect(snapshots_delete).to receive(:execute)
    RDS.new(options).snapshots_delete(db_instance_id: db_instance_id, days:days, months: months, years: years)
  end
