...
describe AwsHelpers::Actions::ElasticLoadBalancing::PollInServiceInstances do
  ...
  describe '#execute' do
    ...
    it 'should call describe_instance_health with correct parameters on the AWS::ElasticLoadBalancing::Client' do
      ...
      AwsHelpers::Actions::ElasticLoadBalancing::PollInServiceInstances.new(stdout, config, [load_balancer_name], 1, 0).execute
    end

    it 'should write to std_out the load balancer name' do
      response = create_response
    ...
    it 'should write to std_out the number of servers in various states' do
      ...
      AwsHelpers::Actions::ElasticLoadBalancing::PollInServiceInstances.new(stdout, config, [load_balancer_name], 2, 0).execute
    end
    ...
    it 'should raise an error if all servers are not in service after max_attempts are exceeded' do
      ...
      expect{AwsHelpers::Actions::ElasticLoadBalancing::PollInServiceInstances.new(stdout, config, [load_balancer_name], 1, 0).execute}.to raise_error(Aws::Waiters::Errors::TooManyAttemptsError)
    end
