...
describe StackEventsFilterPostInitiation do
  ...
  let(:rollback_in_progress_event) { CreateEventHelper.new(stack_name, stack_id, 'ROLLBACK_IN_PROGRESS', resource_type).execute }
  let(:initiation_event) { CreateEventHelper.new(stack_name, stack_id, 'DELETE_IN_PROGRESS', resource_type).execute }
