...
describe S3TemplateUrl do
  ...
  it 'should return true if a s3 bucket exists' do
    ...
    expect(S3TemplateUrl.new(config, s3_bucket_name).execute).to eq('s3-bucket-url')
  end
  ...
  it 'should raise and error if s3 bucket does not exist' do
    ...
    expect { S3TemplateUrl.new(config, s3_bucket_name).execute }.to raise_error('Aws::S3::Errors::NotFound')
  end
