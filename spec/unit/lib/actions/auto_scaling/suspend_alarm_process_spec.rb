...
require 'aws_helpers/actions/auto_scaling/suspend_alarm_process'

describe AwsHelpers::Actions::AutoScaling::SuspendAlarmProcess do
  describe '#execute' do
    ...
    describe '#execute' do
      ...
      it 'should call add_tags with correct parameters on the AWS::ElasticLoadBalancing::Client' do
        expect(auto_scaling_client).to receive(:suspend_processes).with(payload).and_return([])
        AwsHelpers::Actions::AutoScaling::SuspendAlarmProcess.new(config, auto_scaling_group_name, stdout: stdout).execute
      end
