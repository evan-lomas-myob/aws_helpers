...
describe AwsHelpers::Actions::AutoScaling::PollHealthyInstances do
  ...
  describe '#execute' do
    ...
    it 'should call the Aws::AutoScaling::Client #describe_auto_scaling_instances with correct parameters' do
      ...
      AwsHelpers::Actions::AutoScaling::PollHealthyInstances.new(std_out, config, auto_scaling_group_name, 0, 1).execute
    end
    ...
    it 'should output the instance status' do
      ...
      expect(std_out).to receive(:puts).with("#{auto_scaling_group_name} instances. Desired Capacity=1, InService=1")
      AwsHelpers::Actions::AutoScaling::PollHealthyInstances.new(std_out, config, auto_scaling_group_name, 0, 1).execute
    end
    ...
    it 'should poll until all instances are in service' do
      ...
      expect(std_out).to receive(:puts).with("#{auto_scaling_group_name} instances. Desired Capacity=2, InService=1, Pending=1")
      expect(std_out).to receive(:puts).with("#{auto_scaling_group_name} instances. Desired Capacity=2, InService=2")
      AwsHelpers::Actions::AutoScaling::PollHealthyInstances.new(std_out, config, auto_scaling_group_name, 0, 2).execute
    end
    ...
    it 'should throw an error if expected number of servers is not reached by the retry period' do
      ...
      allow(auto_scaling_client).to receive(:describe_auto_scaling_groups).and_return(response)
      expect { AwsHelpers::Actions::AutoScaling::PollHealthyInstances.new(std_out, config, auto_scaling_group_name, 0, 1).execute }.to raise_error(Aws::Waiters::Errors::TooManyAttemptsError)
    end
