...
describe AwsHelpers::Utilities::TargetStackValidate do
  ...
  it 'should return the stack name' do
    expect(AwsHelpers::Utilities::TargetStackValidate.new.execute(stack_name_options)).to eq(stack_name)
  end

  it 'should return the stack id' do
    expect(AwsHelpers::Utilities::TargetStackValidate.new.execute(stack_id_options)).to eq(stack_id)
  end

  it "should raise exception if a stack name or stack id doesn't exist" do
    expect { AwsHelpers::Utilities::TargetStackValidate.new.execute({}) }.to raise_error(RuntimeError, 'You must supply and options[:stack_name] or options[:stack_id]')
  end
